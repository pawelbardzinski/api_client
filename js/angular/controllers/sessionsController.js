app.controller('sessionsController', ['$scope', '$http', '$timeout', '$cookies', function($scope, $http, $timeout, $cookies) {
  ...
  $scope.destroy = function(id) {
    ...
    var rejected = function(response) {
      if (response.status == 401) {
        $scope.logOut();
        $scope.errorMessage = "Your session expired, you have been logged out";
      }
    }
  ...
  $scope.createList = function() {
    ...
    var rejected = function(response) {
      $scope.logOut();
      $scope.errorMessage = "There was a problem with saving data; you are logged out for security reasons";
    }
